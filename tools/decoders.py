...
def make_k_decoders(X, phi, k=4, full_return=False):
    ...
    )  # TODO: can only do this if I am taking one bin at the time, otherwise history doesn-t make sense
    for train_index, test_index in kf.split(X):
        # Get the training and testing data
        X_train, Y_train = (
            X[train_index, :],
            [sin_phi[train_index], cos_phi[train_index]],
        )
        X_test, Y_test = X[test_index, :], [sin_phi[test_index], cos_phi[test_index]]

        # Put them in the right format
        ## Get the history
        X_train_ = get_spikes_with_history(
            X_train, bins_before, bins_current, bins_after
        )
        ...
        ## Reshape
        X_train_flat = X_train_.reshape(
            X_train_.shape[0], X_train_.shape[1] * X_train_.shape[2]
        )
        Y_train = np.array(Y_train).T
        X_test_flat = X_test_.reshape(
            X_test_.shape[0], X_test_.shape[1] * X_test_.shape[2]
        )
        ...
        CMSEs.append(
            circular_mean_squared_error(
                sin_true, sin_predicted, cos_true, cos_predicted
            )
        )
...
def get_spikes_with_history(neural_data, bins_before, bins_after, bins_current=1):
    ...
    for i in range(
        ...
        end_idx = (
            start_idx + surrounding_bins
        )  # The bins of neural data we will be including are between start_idx and end_idx (which will have length "surrounding_bins")
