...
def combine_files_to_csv(animal, fov, experiment, run):
    ...
    print('...start loading the files for: '+animal+'_'+fov+'_'+experiment+'-'+run)
    ...
    # Add behavior data
    beh_df = pd.DataFrame({'animal': animal,
...
def get_rois_to_exclude(animal, fov, experiment, run):
    ...
    for file in txt:
        ...
        if (file.split(',')[0] == 'list_'+animal+'_'+fov+'_'+experiment+'-'+run+'.txt') or (file.split(',')[0] == 'list_'+animal+'_'+experiment+'-'+run+'.txt'):
            try:
                rois = file.split(',')[-1]
...
def add_movements_to_csv(animal, fov, experiment, run, moving_threshold=20):
    ...
    df_spikes.insert(df_spikes.columns.get_loc('y')+1, 'movement_status', movement_status)
