...
def get_smoothed_moving_spikes(
    animal,
    fov,
    experiment,
    run,
    ...
    * Removes the stationary points.
    ...
    while np.isnan(spikes).any().sum() > 0:
        if np.isnan(spikes).any(axis=0).sum() == spikes.shape[1]:
            ...
            print(
                f"WARNING: Removing time-points with NaN values from {animal}_{fov}_{experiment}-{run}: from {spikes.shape[0]} to {np.sum(~np.isnan(spikes).any(axis=1))}"
            )
        ...
        if np.isnan(spikes).any(axis=1).sum() == spikes.shape[0]:
            ...
            print(
                f"WARNING: Removing neurons with NaN values from {animal}_{fov}_{experiment}-{run}: from {spikes.shape[1]} to {np.sum(~np.isnan(spikes).any(axis=0))}"
            )
    ...
    moving_masks_b = (
        np.sum(
            moving_masks[: n_bins * bins_compress].reshape(-1, bins_compress), axis=1
        )
        > 0
    )
...
#####################################################
def get_smoothed_moving_all_data(
    animal, fov, experiment, run, n_points=360, portion_to_remove=0.0
):
    ...
    firing_rates, time, phi, cells = get_smoothed_moving_spikes(
        animal, fov, experiment, run, portion_to_remove=portion_to_remove
    )
    ...
    average_firing_rates, phi_bins = get_tuning_curves(
        firing_rates, phi, n_points=n_points
    )
    # Unwrap the time
    tdiff = np.diff(time)
    time_unwrapped = np.zeros(len(time))
        ...
        for b in range(ring_neural.shape[0]):
            if np.isnan(ring_neural[b, :]).any():
                ...
                if b == 0:
                    ...
                    ring_neural[b, :] = (
                        ring_neural[b - 1, :] + ring_neural[b + 1, :]
                    ) / 2
...
def from_local_to_global_index(animal, fov, session, local_indexes):
    ...
    for li in local_indexes:
        ...
        try:
            ...
            raise IndexError(
                f"Local cell {li} not found in global index for {animal}_{fov}_{session}"
            ) from exc
